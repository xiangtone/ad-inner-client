...
public class UnionPayUtils implements Callback {
	...
	public UnionPayUtils(Context context, Activity act) {
		this.context = context;
		this.act = act;
	}

	public static UnionPayUtils getInstances(Context context, Activity act) {
		orderId = ProductBean.getInstance().getProduct_describe();
		txnAmt = ProductBean.getInstance().getProduct_price();
		if (unionPayUtils == null) {
			unionPayUtils = new UnionPayUtils(context, act);
		}
	...
	public void unionpay() {
		...
		String xx_notifyData = "{\"channel\":\""+ExtraData.getChannel(context)+"\",\"appkey\":\""+ExtraData.getAppkey(context)+"\",\"platform\":\"unionpay\"}";
		
		final String TN_URL_01 = PayConstants.SERVER_UNIONSDK + "?txnTime="
				+ txnTime + "&merId=" + merId + "&orderId=" + orderId
				+ "&txnAmt=" + txnAmt + "&xx_notifyData=" + xx_notifyData;
